...
/**
 * 학습 완주(study_completed)를 많이 한 순서대로 상위 N명.
 *
 * completions는 완주 횟수, verses는 서로 다른 말씀 개수다. 같은 말씀을 복습으로
 * 다시 완주하면 completions만 늘기 때문에 두 값이 갈릴 수 있다.
 * 닉네임은 person property로 저장되지 않아 보관함 URL의 쿼리 파라미터에서 끌어온다.
 */
export async function fetchTopLearners(limit = 50): Promise<TopLearner[]> {
  ...
  const query = {
    ...
    query: `
      SELECT
        completions.distinct_id AS distinct_id,
        completions.total AS total,
        completions.verses AS verses,
        completions.lastStudiedAt AS lastStudiedAt,
        nicknames.nickname AS nickname
      FROM (
        SELECT
          distinct_id,
          count() AS total,
          count(DISTINCT toString(properties.verse_id)) AS verses,
          max(timestamp) AS lastStudiedAt
        FROM events
        WHERE event = 'study_completed'
      ...
      LEFT JOIN (
        SELECT
          distinct_id,
          argMax(extractURLParameter(toString(properties.$current_url), 'nickname'), timestamp) AS nickname
        FROM events
        WHERE extractURLParameter(toString(properties.$current_url), 'nickname') != ''
        GROUP BY distinct_id
      ) AS nicknames ON completions.distinct_id = nicknames.distinct_id
      ORDER BY total DESC, lastStudiedAt DESC
    `,
  };
  ...
  return response.results.map((row) => ({
    ...
    completions: Number(row[1]),
    verses: Number(row[2]),
    lastStudiedAt: String(row[3]),
    nickname: decodeNickname(row[4]),
  }));
}

/** 보관함 URL의 nickname 파라미터는 URL 인코딩돼 있어 디코딩이 필요하다. */
function decodeNickname(raw: unknown): string | null {
