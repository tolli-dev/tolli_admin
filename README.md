...
- `src/lib/appstore/`, `src/lib/playstore/` — App Store 평점(iTunes Lookup API, 무인증) / Google Play 리뷰(서비스 계정) 조회
...
- 앱스토어/플레이 다운로드 수는 연동하지 않았다 — Apple은 Finance 권한이 있는 Sales Reports API가 필요하고, Google Play는 공식 API에 다운로드 수 엔드포인트가 없어 BigQuery 익스포트가 필요하다.
